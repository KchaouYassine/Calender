...
export default function Day({day,heuresArray,minutesArray}){

    const [showButton ,setShowButton] = useState(false);
    const [resultTime,setResultTime] = useState(0);
    const [startTime,setStartTime]  = useState();
    const [endTime,setEndTime]  = useState();
    const [breackTime,setBreackTime]  = useState();
    ...
    const setTimDay =()=>{
        ...
        if(typeof(breackTime) === "string"){
            let breackHeure=Number(breackTime.split(":")[0])
            let breackMin=Number(breackTime.split(":")[1])
            let t3=breackHeure*60-breackMin
            t1-=t3
        }
    ...
    return(    
        <tr className='p-16 text-center border-b-2 border-gray-500 md:text-bold md:text-xl'> 
            ...
            <td style={{width:'18.5%'}}><input className='p-2 bg-transparent'   type="time"   onChange={(e)=>{setBreackTime(e.target.value)}} /></td>
            <td style={{width:'20%'}}>
                {
